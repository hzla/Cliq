...
(function() {
  var Categories, ready;

  Categories = {
    init: function() {
      this.autocompleteQS();
      $('body').on('click', '.cat', this.highlight);
      $('body').on('click', '#next-cat', this.goNext);
      $('body').on('ajax:success', '.cat-link', this.showCategories);
      $('body').on('ajax:success', '.back-link', this.showCategories);
      $('body').on('ajax:success', '.add-act', this.addActivity);
      $('body').on('ajax:success', '.remove-act', this.removeActivity);
      $('body').on('ajax:success', '#new_activity', this.suggestActivity);
      $('body').on('ajax:success', '#quick-search', this.quickAdd);
      $('body').on('click', '#suggest-text', this.prepSuggest);
      return $('body').on('ajax:success', '.top-nav-content a, .extended-side-nav a', this.autocompleteQS);
    },
    prepSuggest: function() {
      $(this).hide();
      $('#activity_name').show();
      $('#new_activity').css('margin-top', '40px');
      if ($('#quick-search:visible').length > 0) {
        return $('#new_activity').css('margin-top', '205px');
      }
    },
    highlight: function() {
      $(this).children('.cat-title').css('color', 'white');
      return $(this).children('.cat-title').css('background-color', '#ffa320');
    },
    goNext: function() {
      return $('.doing').parent().next()[0].click();
    },
    showCategories: function(event, data, xhr, status) {
      var url;
    ...
    addActivity: function(event, data, xhr, status) {
      var likeCount;
      $(this).remove();
    ...
    removeActivity: function(event, data, xhr, status) {
      var likeCount;
      $(this).remove();
    ...
    suggestActivity: function() {
      ...
      return setTimeout(function() {
        return $('#thank-suggestion').remove();
      }, 3000);
    },
    autocompleteQS: function() {
      return $('.quick-search-acts').autocomplete({
        source: '/activities',
        select: function(event, ui) {
          ...
          return $('.interest-info').text(ui.item.id + "@" + ui.item.root_id + "@" + ui.item.cat_name);
        },
        focus: function(event, ui) {
          ...
          return $('.ui-state-focus').removeClass('ui-state-focus').addClass('new-focus');
        },
        delay: 0,
        open: function(event, ui) {
          var $input, $results, height, inputHeight, newTop, top;
          $input = $(event.target);
          ...
          newTop -= 15;
          return $results.css("top", newTop + "px");
        },
        minLength: 2
      });
    },
    quickAdd: function() {
      var id, intCount, likeCount;
      id = "#" + $('#act_id').val();
      $(id).click();
      this.reset();
      $('#added').remove;
      ...
      if ($('.tut').length > 0) {
        ...
        return $('#int-number').text(intCount);
      }
    }
  };
...
(function() {
  ...
  Cliq = {
    init: function() {
      Cliq.nextState = true;
      $('.tab').on('ajax:success', this.search);
      $('.tab').on('click', this.pushState);
      $('.scroll-down').click(this.scrollDown);
      $(window).bind('popstate', this.popState);
      return this.hightlightTab();
    },
    scrollDown: function() {
      return $('.content-container').scrollTo($(window).height());
    },
    changeSlide: function() {
      var slide;
      slide = $(this).attr('id').slice(-1);
      return $.fn.fullpage.moveTo(slide);
    },
    hightlightTab: function() {
      var tab, url;
      ...
      if (url[url.length - 2] === "users") {
        return $('#profile').children('.side-icon').addClass('active');
      }
    },
    popState: function() {
      var tab, url;
      ...
      return $('.back-link').click();
    },
    search: function(event, data, xhr, status) {
    ...
    pushState: function() {
      ...
      if ($('.mobile').length < 1) {
        ...
        return $('.content-container').css('opacity', '.3');
      }
    }
  };
...
(function() {
  ...
  CliqUi = {
    init: function() {
      $('body').on('ajax:success', '.invite-user', this.inviteUser);
      $('body').on('ajax:beforeSend', '.chat-user', this.checkTab);
      $('body').on('click', '.chat-user', this.switchChat);
      $('body').on('ajax:success', '.chat-user', this.chatUser);
      $('body').on('ajax:success', '.other-user', this.showUser);
      if ($('.mobile').length < 1) {
        $('body').on('click', '.chat-collapse', this.collapseChat);
      }
      $('body').on('click', '.collapsed-chat', this.addChat);
      $('body').on('click', '.close', this.closeChat);
      if ($('.mobile').length < 1) {
        $('.content-container').click(this.collapseAllChat);
      }
      if ($('.mobile').length < 1) {
        $('.results-container').click(this.collapseAllChat);
      }
      $('body').on('ajax:success', '.mobile .event-action', this.closeEvent);
      this.switchText();
      if ($('#given-feedback').length < 1) {
        this.bounceFeedback();
      }
      return CliqUi.gameIndex = 0;
    },
    switchText: function() {
      var games;
      games = ["Cards Against Humanity", "intramural sports", "League of Legends", "a dinner party", "Halo", "visiting museums", "joining a fraternity", "watching a play", "getting coffee", "COD", "exploring the outdoors", "joining a band", "playing Smash", "playing Settlers of Catan", "going to the beach", "smoking hookah", "hosting a dinner party", "adopting a puppy", "learning to DJ", "fighting for a cause", "netflix with fellow homebodies", "going a cappella", "Laying down improv", "joining a car club", "Ultimate Frisbee"];
      return setInterval(function() {
        var game;
        CliqUi.gameIndex += 1;
        game = games[CliqUi.gameIndex % 7];
        $('#game').text(game);
        return $('#game').hide().hide().show();
      }, 2000);
    },
    bounceFeedback: function() {
      return setTimeout(function() {
        return setInterval(function() {
          $('#send-feedback').css('background', '#414141');
          return $('#send-feedback').hide().hide().show();
        }, 15000);
      }, 20000);
    },
    closeEvent: function() {
    ...
    swipeAway: function() {
      ...
      return $('.chat-partial').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function() {
        $('.chat-partial').remove();
        $('.content-container').css('opacity', '');
    ...
    switchChat: function() {
      if ($('.conversation').length > 0) {
        ...
        $('#' + $(this).attr('href').split('/')[2]).children('.convo-link').click();
        $('.content-container').css('opacity', '');
        return $('.content-container').css('background', '#f1f1f1');
      }
    },
    showUser: function(event, data, xhr, status) {
      $('.user-other-container').remove();
      $('body').append(data);
      $('.user-other-container').addClass('animated bounceInRight');
      $('.content-container').css('opacity', '.3');
      $('.cat-collection').kinetic();
      return $('#profile-container').swipe({
        swipe: function(event, direction, distance, duration, fingerCount) {
          if (direction === "right") {
            $('.user-other-container').addClass('animated fadeOutRightBig');
            $('.user-other-container').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function() {
              $('.chat-partial').remove();
              $('.content-container').css('opacity', '');
              $('.content-container').css('background', '#f1f1f1');
    ...
    collapseChat: function() {
      return CliqUi.collapse($(this).parents('.chat-partial'));
    },
    collapse: function(chat) {
      var className, collapsed, collapsedCount, indent;
    ...
    collapseAllChat: function() {
      var elapsedTime;
      $('div.chat-partial').not('.bounceOutRight').each(function() {
        if ($('#chat-partial-content')) {
          return CliqUi.collapse($(this));
        }
      });
      ...
      if (CliqUi.counting) {
        ...
        return $.post('/events/discuss_time', {
    ...
    addChat: function() {
      ...
      $(this).one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function() {
        $(this).removeClass('animated bounceInLeft bounceOutLeft shown');
        return $(this).hide();
      });
      $(this).prev().removeClass().addClass('chat-partial animated bounceInRight');
      $('.chat-partial').css('z-index', '2');
      $(this).prev().css('z-index', '3');
      $('.chat-partial').removeClass('current-thread');
      $(this).prev().addClass('current-thread');
    ...
    obscureDate: function() {
      var dates;
      if ($('.no-obscure').length < 1) {
        dates = $('.date');
        CliqUi.obscureDates(dates);
      }
      return $('.date:visible').parents('.message-block').addClass('new');
    },
    obscureDates: function(dates) {
      return dates.each(function(index) {
        var cHours, cMins, cTime, cTotalTime, date, difference, hours, mins, time, totalTime;
        if (dates[index - 1] === void 0) {
          ...
          cTime = $(this).text().substr(date.length - 9).trim().slice(0, 4);
...
(function() {
  var EventTutorial, ready;

  EventTutorial = {
    init: function() {
      $('body').on('click', '#sift-option', this.extendSift);
      $('body').on('click', '#create-option', this.extendcreate);
      $('body').on('click', '.confirmation.got-it', this.finishTut);
      $('body').on('ajax:success', '.confirmation-form', this.checkEmail);
      $('body').on('click', '#tut-1 .confirmation', this.checkUCF);
      return $('body').on('focus', '#email', this.hidePlaceholder);
    },
    hidePlaceholder: function() {
      return $(this).attr('placeholder', '');
    },
    checkUCF: function() {
      if ($('#email').val().slice(-7) === "ucf.edu" || $('#email').val() === "andylee.hzl@gmail.com" || $('#email').val() === "twitchymonster@gmail.com") {
        return $('.confirmation-form').submit();
      } else {
        $('#email').css('border', '1px solid red');
        $('#email').attr('placeholder', 'Please enter a valid UCF email');
        return $('.confirmation-form')[0].reset();
      }
    },
    checkEmail: function() {
      $(this)[0].reset();
      return $('#email').attr("placeholder", "Thank you! Please check your email.");
    },
    finishTut: function() {
      $('#tut-2').hide();
      ...
      return history.pushState(null, document.title, $(window)[0].location.href.replace(/\?.*=true/g, ""));
    },
    extendcreate: function() {
      var create, sift;
      $('.event-sort').first().click();
      $('#events-header-actions').show();
      $(this).find('.option-title, .tut-sift-pic, .option-question').addClass('animated fadeOut');
...
(function() {
  ...
  EventUi = {
    init: function() {
      $('body').on('click', '#new-open-event', this.switchPane);
      $('body').on('click', '#next-form', this.nextForm);
      return $('body').on('click', '.ui-datepicker-current', this.closeDate);
    },
    closeDate: function() {
      console.log("something");
      return $('.ui-datepicker-close').click();
    },
    nextForm: function() {
      var starttime, title;
      ...
      if (title && starttime) {
        ...
        return $('#form-section-2').show().addClass('animated fadeInRight');
      } else {
        if (!title) {
          $('#event_title').css('border', '1px solid red');
        }
        if (!starttime) {
          return $('#event_start_time').css('border', '1px solid red');
        }
      }
    },
    switchPane: function() {
      $(this).toggleClass('sift-state').children(':not(.sift-text, .create-event-text)').toggle();
      $('.pane.inactive').css('top', '-100%');
      $('.pane.inactive').show().animate({
        top: '0'
      }, 500, function() {
        return $(this).removeClass('inactive').addClass('current');
      });
      return $('.pane.current').show().animate({
        top: '100%'
      }, 500, function() {
        return $(this).removeClass('current').addClass('inactive');
      });
    }
  };
...
(function() {
  ...
  Events = {
    init: function() {
      $('#upload-button').click(this.browseFiles);
      $('body').on('ajax:success', '.event-action', this.removeInvite);
      $('body').on('change', '#event_partner_id', this.addPartnerInfo);
      $('body').on('ajax:success', '.event-sort', this.sortEvents);
      $('body').on('ajax:success', '#new_event', this.closeModal);
      $('body').on('click', '#send-invite .invite-icon', this.send);
      $('body').on($.modal.BEFORE_CLOSE, '#invite-modal-container', this.restoreOpacity);
      $('body').on('click', '.form-row div', this.filterEvents);
      $('body').on('click', '.filter-container .form-row div', this.filterEvents);
      $('body').on('click', '.event-filter', this.bigFilter);
      return Events.posting = 0;
    },
    bigFilter: function() {
      if ($('.mobile').length > 0) {
      ...
      if ($('#event-hosting').parent().hasClass('selected')) {
        $('.public-event-create').show();
      }
      if ($('#event-open').parent().hasClass('selected')) {
        $('.ev-overlay').css('opacity', '');
      }
    ...
    filterEvents: function() {
      var afterTags, currentTags, tags;
      currentTags = $('.filter-on .event-attr.selected .attr-text').map(function() {
      ...
      if (afterTags[afterTags.length - 1] === ".") {
        afterTags = afterTags.slice(0, afterTags.length - 1);
      }
      if ($('.mobile').length < 1) {
      ...
      if (Events.posting % 2 === 1 && $('.mobile').length < 1) {
        return $.post('/events/toggle', function(data) {
          return console.log("posted");
        });
      }
    },
    send: function() {
      return $('#new_event').submit();
    },
    browseFiles: function() {
      return $('#event_image').click();
    },
    removeInvite: function() {
      ...
      if ($(this).hasClass('pass')) {
        ...
        return invite.one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function() {
          return invite.animate({
            height: "0px"
          }, 200, function() {
            return invite.hide();
          });
        ...
        return invite.find('.view-thread').show();
      }
    },
    restoreOpacity: function(event, modal) {
      if ($('.user-other-container:visible').length > 0 || $('.chat-partial:visible').length > 0) {
        return $('.content-container').css('opacity', '.3');
      }
    },
    addPartnerInfo: function() {
      var partnerId;
      partnerId = $('#event_partner_id')[0].value;
      return $.get('/partners/' + partnerId + '.json', function(data) {
        ...
        $('#sat').text(data.sat);
        $('#sun').text(data.sun);
        return $('#invite-modal-container').css("margin-top", parseInt($('#invite-modal-container').css('margin-top').slice(0, -2)) - 30 + "px");
      });
    },
    sortEvents: function(event, data, xhr, status) {
      var eventType;
      eventType = $(this).children().text();
    ...
    closeModal: function(event, data, xhr, status) {
      if (data.ok === true) {
        $.modal.close();
        ...
        if (data.location) {
          $('#event_location').css('border', '1px solid red');
        }
        if (data.start_time) {
          return $('#event_start_time').css('border', '1px solid red');
        }
    ...
    allotTimes: function() {
      ...
      dates = $('.upcoming-event:not(.today, .tomorrow)').map(function() {
        return $(this).attr('class').split(' ')[1];
      });
      dates = $.makeArray($.unique(dates));
      $.each(dates, function() {
        var date, splitDate;
        splitDate = this.split("-").join(" ");
        date = this.charAt(0).toUpperCase() + this.slice(1);
        return $('.event-date-type').last().after("<div class='event-date-type " + date + "'><div class='date-header head-2'>" + splitDate + "</div></div>");
      });
      return $.each(dates, function() {
        return $(".event-date-type." + this).append($(".upcoming-event." + this + ":not(.today, .tommorow)"));
      });
    }
  };
...
(function() {
  ...
  EventForm = {
    init: function() {
      $('body').on('click', '.event-type', this.chooseType);
      $('body').on('click', '.event-attr', this.chooseTag);
      $('body').on('click', '.event-requirement', this.chooseRequirement);
      return $('body').on('submit', '#new_event', this.checkFields);
    },
    checkFields: function(event) {
      var emptyTime, emptyTitle;
      emptyTitle = $('#event_title').val() === "";
      emptyTime = $('#event_start_time').val() === "";
      if (emptyTime || emptyTitle) {
        event.preventDefault();
      }
      if (emptyTitle) {
        $('#event_title').css('border', '1px solid red');
    ...
    chooseType: function() {
      var selected, type;
      if ($(this).parents('#invites').length > 0 || $(this).parents('.filter-container').length > 0) {
        selected = $(this).hasClass('selected');
        $(this).parent().find('.event-type.selected').removeClass('selected');
        if (!selected) {
          $(this).addClass('selected');
        }
        return;
      ...
      $(this).addClass('selected');
      type = $(this).text();
      if ($('#event_event_type').length > 0) {
        return $('#event_event_type').val(type);
      }
    },
    chooseTag: function() {
      var currentTags;
      $(this).toggleClass('selected');
      currentTags = $('.event-attr.selected .attr-text').map(function() {
        return $(this).text().replace("Drinks", "Party").split(' ').pop();
      });
      if ($('.tags').length > 0) {
        $('.tags').val($.makeArray(currentTags).join());
      }
      return $(this).addClass("stupid-safari").removeClass("stupid-safari");
    },
    chooseRequirement: function() {
      var currentReqs;
      $(this).toggleClass('selected');
      currentReqs = $('.event-requirement.selected .require-text').map(function() {
        return $(this).text();
      });
      return $('.requirements').val($.makeArray(currentReqs).join());
    }
  };

  ready = function() {
    return EventForm.init();
  };
...
(function() {
  ...
  Game = {
    init: function() {
      $('.content-container').css('background', 'white');
      $('input').keypress(this.evaluate);
      $('.number-input').keyup(this.numbersOnly);
      $('.op-input').keyup(this.opsOnly);
      this.ops = ["+", "-", "*", "/", "^", "%"];
      $('.start').click(this.countDown);
      $('.item-img').click(this.addImg);
      $('.other-start').click(this.otherCD);
      $('.buy').click(this.calc);
      return $('.give-up').click(this.giveUp);
    },
    giveUp: function() {
      return $('.timer').text(':1');
    },
    calc: function() {
      var amount, bought, have, shit, total, type;
      total = 0;
      shit = "";
    ...
    otherCD: function() {
      if (!Game.countingDown) {
        $('.start-overlay').hide();
        return Game.int = setInterval(function() {
          var newVal;
          newVal = parseInt($('.timer').text().slice(-2)) - 1;
          if ($('.timer').text().length === 2) {
            newVal = parseInt($('.timer').text().slice(-1)) - 1;
          }
          if (newVal === 0) {
            $.get('/lost');
            location.reload();
    ...
    opsOnly: function() {
      var numbers;
      numbers = $('.number').map(function() {
        return $(this).text();
      });
      if ($.inArray(this.value.slice(-1), numbers) !== -1) {
        $(this).next().val(this.value.slice(-1));
        this.value = this.value.slice(0, -1);
        $(this).next().next().trigger('focus');
        return;
      }
      if ($.inArray(this.value, Game.ops) === -1) {
        return this.value = '';
      }
    },
    numbersOnly: function() {
      var index, numArray, numbers, usedNumbers, value;
      usedNumbers = $('.number-input').map(function() {
        return $(this).val();
      });
      ...
      numArray.push(")");
      numArray.push("!");
      if ($.inArray(this.value.slice(-1), Game.ops) !== -1) {
        $(this).next().val(this.value.slice(-1));
        this.value = this.value.slice(0, -1);
        $(this).next().next().trigger('focus');
      }
      value = this.value;
      index = usedNumbers.indexOf(this.value);
      usedNumbers = usedNumbers.filter(function(x) {
        return x !== value.toString();
      });
      if (($.inArray(this.value, numArray) === -1 && this.value.slice(0, 1) !== "(" && this.value.slice(-1) !== ")" && this.value.slice(-1) !== "!" && this.value.slice(-2, -1) !== "!") || usedNumbers.length < 3) {
        return this.value = '';
      }
    },
    evaluate: function() {
      return setTimeout(function() {
        var answer, numbers, ops, regex, wholeArray;
        numbers = $('.number-input').map(function() {
          return $(this).val();
        });
        numbers = $.makeArray(numbers).filter(function(x) {
          return x !== "";
        });
        ops = $('.op-input').map(function() {
          return $(this).val();
        });
        ops = $.makeArray(ops).filter(function(x) {
          return x !== "";
        });
        console.log(ops);
        console.log(numbers);
        ...
        if (ops.length === 3 && numbers.length === 4) {
          ...
          regex = /(\d)!/g;
          wholeArray = wholeArray.replace(regex, function(match, number) {
            return Game.fact(parseInt(number));
          });
...
(function() {
  ...
  Messages = {
    init: function() {
      ...
      $('body').on('click', '.reply', this.submitMessage);
      $('body').on('click', '.convo-link', this.prepConversation);
      $('body').on('ajax:success', '.convo-link', this.appendConversation);
      $('body').on('keypress', '#chat-box', this.submitMessageOnEnter);
      $('#messages-container').parent().parent().css('background', 'white');
      $('body').on('ajax:success', '.block', this.blockUser);
      return $('.conversation').hover(this.showBlock, this.hideBlock);
    },
    showBlock: function() {
      return $(this).find('.convo-block').show();
    },
    hideBlock: function() {
      return $(this).find('.convo-block').hide();
    },
    submitMessage: function() {
      if ($('.mobile').length < 1) {
        $(this).parents('.chat-window-footer').find('form').submit();
        if ($('#chat-box').val().replace(/^\s+|\s+$/g, "") !== "") {
          $('.sending-progress').text('Sending...').show();
        }
        return $('.chat-box').val('');
      }
    },
    prepConversation: function() {
      $('.conversation').removeClass('current');
      $(this).parent().addClass('current');
      return $('#messages-box').html('loading...');
    },
    appendConversation: function(event, data, xhr, status) {
      if ($('.mobile').length < 1) {
        $('#messages-box').html(data);
        $('.message-history')[0].scrollTop = $('.message-history')[0].scrollHeight;
        $('.convo-link').find('.convo-last-message').each(function() {
          if ($(this).css('color') !== "rgb(24, 195, 189)") {
            return $(this).css('color', '#bebebe');
          }
        });
        return $(this).find('.convo-last-message').css('color', 'white');
      }
    },
    submitMessageOnEnter: function(e) {
      var form;
    ...
    blockUser: function() {
      $(this).parents('.conversation').remove();
      return $('#messages-box').html('');
    }
  };
...
(function() {
  var MobileEvents, ready;

  MobileEvents = {
    init: function() {
      $('body').on('ajax:success', '.event-bar-link', this.showEvents);
      $('body').on('ajax:success', '.public-event-create', this.showEventForm);
      $('body').on('ajax:success', '#new_event', this.closeForm);
      $('body').on('ajax:success', '.public-event-create, #event-list .chat-user', this.showBack);
      $('body').on('touchend', '.event-back', this.closeEventModals);
      $('body').on('click', '.filter-link', this.showFilters);
      return $('body').on('click', '.filter-button', this.liftFilter);
    },
    liftFilter: function() {
      $('.filter-container').removeClass('animated bounceInDown').addClass('animated bounceOutUp');
      $('.filter-container').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function() {
        return $(this).hide();
      });
      return $('.menu-title').text('Cliq').css('font-family', 'kari-pro').css('font-style', 'italic').css('font-size', '25px');
    },
    showFilters: function() {
      $('.filter-container').show().removeClass('animated bounceOutUp').addClass('animated bounceInDown');
      return $('.menu-title').text('Sift').css('font-family', 'proxima-nova').css('font-style', 'normal').css('font-size', '17px');
    },
    closeEventModals: function() {
    ...
    showBack: function() {
      $('.back-tut').show();
      return $('.menu-icon.left').hide();
    },
    closeForm: function(event, data, xhr, status) {
      if (data.ok === true) {
        $.modal.close();
        $('#event-form').addClass('animated bounceOutUp');
        $('#event-form').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function() {
          $(this).remove();
          return $('.event-bar-link').first().click();
        });
        return '.message-content'.show();
      }
    },
    showEventForm: function(event, data, xhr, status) {
      ...
      return $('#event-form').html(data).addClass('animated bounceInDown');
    },
    showEvents: function(event, data, xhr, status) {
      $('#event-list').html(data);
      $('.conversation').addClass('animated fadeIn');
      MobileEvents.closeEventModals();
      return MobileEvents.showFilters();
    }
  };

  ready = function() {
    return MobileEvents.init();
  };
...
(function() {
  var MobileMessages, ready;

  MobileMessages = {
    init: function() {
      $('body').on('click', '.mobile .convo-link', this.swipeToConvo);
      $('body').on('ajax:success', '.mobile .convo-link', this.appendConvo);
      $('body').on('ajax:success', '#new_message', this.obscurelastDates);
      $('body').on('ajax:success', '.chat-user', this.obscureDate);
      $('body').on('ajax:success', '.chat-user', this.obscurelastDates);
      $('body').on('click', '.mobile .reply', this.submitMessage);
      $('body').on('click', '.mobile .content-container', this.removeChat);
      $('body').on('click', '.mobile .results-container', this.removeChat);
      $('body').on('touchend', '.back-tut.close-chat', this.removeChat);
      return $('body').on('ajax:success', '.convo-link', this.obscureDate);
    },
    removeChat: function() {
      $('.chat-partial').remove();
      $('.content-container').css('opacity', '');
      ...
      $('.menu-title').text('Cliq');
      return $('.close-chat').remove();
    },
    submitMessage: function() {
      $('#new_message').submit();
      $('#new_message')[0].reset();
      $('.message-history')[0].scrollTop = 0;
      return $('.message-history')[0].scrollTop = $('.message-history')[0].scrollHeight;
    },
    clickConvoLink: function() {
      return $(this).find('.convo-link').click();
    },
    swipeToConvo: function() {
      return $('.message-content').removeClass().addClass('animated fadeOutLeftBig message-content');
    },
    swipeToMessages: function() {
      $('.message-content').removeClass('animated fadeOutLeftBig').addClass('animated fadeInLeftBig');
      $('#thread-container').removeClass().addClass('animated fadeOutRightBig').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function() {
        return $(this).hide();
      });
      return $('.menu-title').text("Cliq");
    },
    appendConvo: function(event, data, xhr, status) {
      var pic;
      pic = $(this).prev().find('.convo-pic').clone();
      $('.menu-title').html(pic);
      ...
      $(this).find('.convo-last-message').css('color', 'white');
      MobileMessages.obscureDates($('.date'));
      return $("#messages-box").swipe({
        swipe: function(event, direction, distance, duration, fingerCount) {
          if (direction === "right") {
            return MobileMessages.swipeToMessages();
          }
        },
        threshold: 150,
        allowPageScroll: "vertical"
    ...
    obscureDate: function() {
      ...
      dates = $('.date');
      lastDates = dates.slice(dates.length - 2, dates.length);
      return MobileMessages.obscureDates(dates);
    },
    obscureDates: function(dates) {
      if ($('.no-obscure').length < 1) {
        return dates.each(function(index) {
...
(function() {
  var MobileSearch, ready;

  MobileSearch = {
    init: function() {
      $('body').on('ajax:success', '#search-form.mobile:not(.big-search)', this.swipeUpToResults);
      $('body').on('click', '.search-icon', this.swipeDownToSearch);
      return $('body').on('click', '.quick-search-button', this.submitSearch);
    },
    submitSearch: function() {
      return $('#search-form').submit();
    },
    swipeUpToResults: function(event, data, xhr, status) {
      ...
      if ($('#empty:visible').length > 0) {
        console.log("no one");
        $('#no-one-around').show();
        return $("#results").swipe({
          swipe: function(event, direction, distance, duration, fingerCount) {
            console.log(distance);
            if (direction === "up") {
              return MobileSearch.swipeDownToSearch();
            }
          },
          threshold: 130
        });
      } else {
        return $(".swiped-actions").swipe({
          swipe: function(event, direction, distance, duration, fingerCount) {
            if (direction === "up") {
              return MobileSearch.swipeDownToSearch();
            }
...
(function() {
  ...
  TopNav = {
    init: function() {
      $('body').on('click', '.menu-icon.left', this.openNav);
      $('body').on('ajax:success', '.top-nav-content a', this.closeNav);
      return $('.content-container').click(this.closeNav);
    },
    openNav: function() {
    ...
    closeNav: function() {
      if ($('.message').length < 1 && $('.event-modal:visible').length < 1) {
        ...
        $('.content-container').css('opacity', '');
        $('.content-container').css('background', '#f1f1f1');
        return $('.content-container').css('position', 'relative');
      }
    }
  };
...
(function() {
  var MobileUsers, ready;

  MobileUsers = {
    init: function() {
      $('body').on('hover', '.mobile .act-name.other', this.clickAct);
      $('body').on('click', '.settings-label', this.toggleEnabled);
      $('body').on('ajax:success', '.edit_user', this.showSaved);
      $('body').on('submit', '.edit_user', this.showSaving);
      $('body').on('touchstart', '.overlay', this.showScrollActs);
      $('body').on('touchstart', '.scroll-cat-name', this.showScrollActs);
      $('body').on('ajax:success', '.mobile .add-other-act', this.addChosen);
      $('body').on('focus', '.mobile .profile-actions input', this.tellEnter);
      $('body').on('focusout', '.mobile .profile-actions input', this.revert);
      $('body').on('submit', '.mobile .profile-actions form', this.focusOut);
      $('body').on('click', '#save-settings-button', this.submitSettings);
      $('body').on('scroll', '.all-scroll-acts', this.scrollDown);
      $('body').on('ajax:success', '.profile-actions form, #cat-header-content form', this.feedback);
      return $('body').on('submit', '#feedback-form', this.thankUser);
    },
    thankUser: function() {
      $('#send-fb').val("Thank you!!");
      return setTimeout(function() {
        return $('#send-fb').val("Send");
      }, 2000);
    },
    feedback: function() {
      var fb;
      if ($(this).attr('id') === "new_activity") {
        fb = "Thank You!";
      } else {
        fb = "Added!";
      }
      $(this).find('.fb-enabled').val(fb).css('color', '#1dcfac');
      return setTimeout(function() {
        return $('.fb-enabled').val('').css('color', '#414141');
      }, 750);
    },
    scrollDown: function() {},
    submitSettings: function() {
      return $('.edit_user').submit();
    },
    tellEnter: function() {
      MobileUsers.old = $(this).attr("placeholder");
      $(this).attr("placeholder", "submit with 'search/go' key");
      $(this).css("width", "calc(90% - 20px)");
      $('.profile-actions').find('form').hide();
      return $(this).parents('form').show();
    },
    revert: function() {
      $(this).attr("placeholder", MobileUsers.old);
      $(this).css("width", "35%");
      return $('.profile-actions').find('form').show();
    },
    focusOut: function() {
      return $(this).find('input').trigger('focusout');
    },
    clickAct: function() {
      return $(this).click();
    },
    ...
    showSaved: function() {
      ...
      return setTimeout(function() {
        return $('.save-text').text("Save Settings");
      }, 2000);
    },
    showSaving: function() {
      return $('.save-text').text("Saving...");
    },
    showScrollActs: function() {
      var acts, clone, id, index;
      if ($('.clone').length > 0) {
        $('.clone').html($('.all-scroll-acts').children());
        $('.clone').removeClass('clone');
      }
      clone = $(this).parent().next().addClass('clone');
      acts = $(this).parent().next().children().clone();
      index = $('.scroll-cat').index($(this).parent()) - 1;
      ...
      $(this).parent().find('.overlay').addClass('orange-overlay');
      id = $(this).parent().attr('id').slice(2);
      return $('#activity_category_id').val(id);
    },
    addChosen: function() {
      if ($('.tut').length < 1) {
        $(this).parent().toggleClass('chosen');
      }
      if ($('.user-other-container:visible').length > 0) {
        return $(this).parent().css('background', '#14a29d');
      }
    }
  };

  ready = function() {
    return MobileUsers.init();
  };
...
(function() {
  ...
  Search = {
    init: function() {
      ...
      this.autocompleteInterests();
      $('.tab').on('ajax:success', this.autoCompleteAll);
      $('body').on('ajax:success', '#search-form', this.displayResults);
      $('body').on('ajax:success', '#search-form', this.getResultInfo);
      $('body').on('submit', '#search-form', this.displaySearching);
      $('.send-activation').on('submit', this.thankUser);
      $('body').on('click', '.searched', this.removeTerm);
      $('body').on('keypress', '#activity', this.selectInterestOnEnter);
      $('body').on('click', '.cliq-invite', this.inviteFriends);
      $('body').on('ajax:success', '#friend-invite', this.sendCliqInvite);
      return $('body').on('keydown', '#query-location', this.deleteLocID);
    },
    autoCompleteAll: function() {
      Search.autocompleteLocations();
      return Search.autocompleteInterests();
    },
    autocompleteLocations: function() {
      return $('#query-location').autocomplete({
        source: '/locations',
        select: function(event, ui) {
          event.preventDefault();
          $(this).val(ui.item.label);
          return $('#location_id').val(ui.item.value);
        },
        focus: function(event, ui) {
          event.preventDefault();
      ...
      if ($('#activity').length > 0) {
        return $('#activity').autocomplete({
          ...
          open: function(event, ui) {
            ...
            return $('.ui-autocomplete').css('width', '+=50px');
          },
          minLength: 2,
    ...
    displayResults: function(event, data, xhr, status) {
      if ($('.mobile').length < 1 || $('.enable-auto-search').length > 0) {
        $('#search-results').html(data);
        $('.result').addClass('animated fadeIn');
        $('#results').html(data);
        $('.swiped-result').hide();
        $('.swiped-result').each(function(index) {
          var empty;
          if (index !== 0 && index % 10 === 0) {
            empty = $('#empty').clone();
            return $(this).replaceWith(empty);
          }
        });
        ...
        if ($('.searched').length < 1 && $('.mobile').length > 0) {
          return $('body').scrollTo($('.mobile .result'));
        }
      }
    },
    getResultInfo: function() {
      return $('.result, .swiped-result').each(function() {
        var id, result;
        if (!$(this).hasClass('no-complete')) {
          id = $(this).attr('id').slice(2);
          result = $(this);
          return $.get("users/" + id + "/result_info", function(data) {
            result.find('.mut-stat').text(data.mut);
            return result.find('.sim-stat').text(data.sim);
          });
        }
      });
    },
    displaySearching: function() {
      return $('#search-results').html("<div id='searching'>SEARCHING...</div>");
    },
    thankUser: function(event, data, xhr, status) {
      if ($('#email').val().match(/.+@.+\..+/i) !== null) {
        ...
        $('#top').css('right', '60px');
        return $('#ty').addClass('animated fadeIn');
      } else {
        $('#top').html("");
    ...
    selectInterestOnEnter: function(e) {
      if ((e.which && e.which === 13) || (e.keyCode && e.keyCode === 13)) {
        return $('.ui-menu-item:visible').first().click();
      }
    },
    inviteFriends: function() {
      var box;
      box = $(this).parents('.no-one-around');
      box.find('.pre-invite').hide();
      box.find('.post-invite').show();
      return box.css('height', '570px');
    },
    sendCliqInvite: function() {
      $(this)[0].reset();
      $('.sent-invite').show().addClass('animated fadeIn');
      return setTimeout(function() {
        return $('.sent-invite').hide();
      }, 1000);
    },
    deleteLocID: function() {
      var key;
...
(function() {
  ...
  Animations = {
    init: function() {
      ...
      $('.extended-options').mouseleave(this.hideOptions);
      return $('.content-container').click(this.hideOptions);
    },
    hideOptions: function() {
      $('.extended-options').hide().find('.icon-label').css('opacity', '0');
      return $('.extended-options').stop().animate({
        width: "62px"
      }, 200);
    },
    ...
    hideLabel: function() {
      $(this).find('.icon-label').css('opacity', '0');
...
(function() {
  ...
  Swipe = {
    init: function() {
      $('body').on('submit', '.search-form', this.hideResults);
      $('body').on('click', '.swipe-next', this.swipeNext);
      $('body').on('click', '.cliq-skip', this.swipeNext);
      $('body').on('ajax:success', '#friend-invite', this.showKeepGoing);
      return this.hideResults();
    },
    hideResults: function() {
      return $('.swiped-result').first().show();
    },
    showKeepGoing: function() {
      return $('.post').show();
    },
    swipeNext: function() {
      var current, next;
      ...
      current.bind('oanimationend animationend webkitAnimationEnd', function() {});
      current.hide();
      $('.post').hide();
      if (next.attr('class') === 'swiped-result' || next.attr('id') === "empty") {
        next.show();
        return next.addClass('animated slideInLeft');
      } else {
        $('.empty').last().show();
        ...
        return $('.empty').last().find('.cliq-skip').hide();
      }
    }
  };
...
(function() {
  ...
  Tutorial = {
    init: function() {
      $('body').on('touchend', '.char', this.highlightChar);
      $('body').on('touchend', '.proceed', this.submitTutorial);
      $('body').on('ajax:success', '.mobile .add-other-act', this.updateInterestCount);
      $('body').on('touchend', '.tut-header.finished', this.welcome);
      return $('body').on('submit', '#tutorial-form', this.checkFields);
    },
    highlightChar: function() {
      var labels, selected;
      if (($('.highlighted').length < 2 && !$(this).hasClass('highlighted')) && !$(this).hasClass('char-button')) {
      ...
      labels = [];
      selected = $('.highlighted .char-label').each(function() {
      ...
      if ($('.highlighted').length === 0) {
        ...
        $('.next-tut').hide();
        return $('.char-button').removeClass('proceed');
      }
    },
    checkFields: function(event) {
      if ($('#characters').val() === "" || $('#tut-school').val() === "") {
        return event.preventDefault();
      }
    },
    submitTutorial: function() {
      return $('#tutorial-form').submit();
    },
    updateInterestCount: function() {
      var difference, intCount, newCount, oldCount;
...
(function() {
  ...
  Users = {
    init: function() {
      $('body').on('click', '#got-it', this.closeWelcome);
      $('#send-feedback').on('click', this.openFeedback);
      $('.content-container').on('click', this.hideFeedback);
      $('#faq-container').on('click', this.hideFeedback);
      $('#feedback-form').on('ajax:success', this.closeFeedback);
      $('#feedback-form').on('submit', this.showSending);
      $('body').on('ajax:success', '.edit_user', this.showSettingsStatus);
      $('body').on('click', '#delete-account', this.showDeleteModal);
      $('body').on('click', '#cancel-delete', this.cancelDelete);
      if ($('.mobile').length < 1) {
        $('body').on('mouseenter', '.act-name.other', this.showPlus);
      }
      if ($('.mobile').length < 1) {
        $('body').on('mouseleave', '.act-name.other', this.showDot);
      }
      $('body').on('ajax:success', '.add-other-act', this.addAct);
      $('body').on('click', '.shared-ints', this.showShared);
      $('body').on('click', '.different-ints', this.showDifferent);
      $('body').on('click', '.all-ints', this.showAll);
      $('body').on('click', '#quick-add', this.prepQuickAdd);
      return $('body').on('submit', '#quick-search', this.addInterest);
    },
    addInterest: function() {
      var catName, id, root_id;
      id = "#c-" + $('.interest-info').text().split("@")[0];
      if ($(id).length > 0) {
        return $(id).append("<div class='activity'><div class='act-name'>" + ($('#self-search').val()) + " •</div></div>");
      } else {
        root_id = "#c-" + $('.interest-info').text().split("@")[1];
        catName = $('.interest-info').text().split("@")[2];
        return $(root_id).append("<div class='cat-group-self'><div class='category dark-gray'><div class='cat-name'>" + catName + "</div> </div><div class='activities' id='" + id + "'><div class='act-name'>" + ($('#self-search').val()) + " •</div></div></div>");
      }
    },
    closeWelcome: function() {
      ...
      if ($('#email').val().slice(-7) === "ucf.edu") {
        $('.welcome-form').submit();
        return $('#activation-container').hide();
      } else {
        return $('#email').css('border', '1px solid red');
      }
    },
    openFeedback: function() {
      $(this).hide();
      return $('#feedback').show().addClass('animated fadeInUp');
    },
    hideFeedback: function() {
      $('#feedback').hide();
      return $('#send-feedback').show();
    },
    ...
    showSending: function() {
      this.reset();
      return $('#feedback-message').text('Thank you for your feedback.');
    },
    showSettingsStatus: function(event, data, xhr, status) {
      if (data.error) {
        console.log("Error");
        return $('#user_address').css('border', '2px solid #ff5959');
      } else {
        if ($('.mobile').length < 1) {
        ...
        return setTimeout(function() {
          return $('#settings-success').remove();
        }, 2000);
      }
    },
    showDeleteModal: function() {
      return $('#delete-modal').modal();
    },
    ...
    addAct: function() {
      return $(this).find('.act-name').addClass('orange shared').removeClass('different');
    },
    showShared: function() {
      ...
      $('.cat-group').show();
      return $('.cat-group').children('.activities').each(function() {
        if ($(this).find('.act-name:visible').length < 1) {
          return $(this).parent().hide();
        }
      });
    },
    showDifferent: function() {
      $('.int-type').removeClass('selected');
      $(this).addClass('selected');
    ...
    showAll: function() {
      ...
      $(this).addClass('selected');
      $('.activity').show();
      return $('.cat-group').show();
    },
    prepQuickAdd: function() {
      $(this).hide();
      $('#quick-search').show();
      return $('#new_activity').css('margin-top', '195px');
    }
  };
